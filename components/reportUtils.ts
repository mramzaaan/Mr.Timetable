    ...
    for (let i = 0; i < totalPages; i++) {
        ...
        for (let pIdx = 0; pIdx < maxPeriods; pIdx++) {
            chunkDays.forEach((day, dIdx) => {
                ...
                const groupTextClass = isGroupPeriod ? ' period-group-text' : '';
                
                const content = `<div class="period-card-img ${colorClass}">${triangleHtml}${separatorHtml}<div class="period-content-spread${groupTextClass}"><div class="period-subject" style="${subjectBadgeStyle}" title="${subjectsList.replace(/<[^>]*>/g, '')}">${subjectsList}</div><div class="period-teacher" title="${teachersList.replace(/<[^>]*>/g, '')}">${teachersList}</div></div></div>`;
