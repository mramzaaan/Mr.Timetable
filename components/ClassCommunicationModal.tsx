...
export const ClassCommunicationModal: React.FC<ClassCommunicationModalProps> = ({
  ...
  const [teacherPos, setTeacherPos] = useState({x: 0, y: 0});
          ...
          * { 
          ...
          ${allColorClasses.map(name => {
              ...
              return `
                ...
                .${name} .period-subject, .${name} .period-teacher { 
                ...
                ${cardStyle === 'badge' ? `
                    .${name} .period-subject { ${badgeTarget === 'subject' ? `background-color: ${textColor}; color: #fff !important; padding: 4px 16px; border-radius: 16px; display: block; width: calc(100% - 16px); text-align: left; box-sizing: border-box; margin: 8px auto 4px auto;` : ''} }
                    .${name} .period-teacher { ${badgeTarget === 'teacher' ? `background-color: ${textColor}; color: #fff !important; padding: 4px 16px; border-radius: 999px; display: block; width: 100%; text-align: right; box-sizing: border-box; margin-bottom: 0; margin-top: auto;` : ''} ${badgeTarget === 'subject' ? `padding: 4px 16px 8px 16px; text-align: right; width: 100%; box-sizing: border-box; margin-top: auto;` : ''} }
                ` : ''}
              `;
          }).join('\n')}
              ...
              if (Array.isArray(slot) && slot.length > 0) {
                  ...
                  if (cardStyle === 'badge') {
                      if (badgeTarget === 'teacher') {
                         teacherBadgeStyle += ` background-color: ${TEXT_HEX_MAP[colorName] || '#000'}; color: #fff !important; padding: 4px 16px; border-radius: 999px; display: block; width: 100%; text-align: right; box-sizing: border-box; margin-bottom: 0; margin-top: auto;`;
                      } else {
  ...
  const handleSendImageAsPicture = async () => {
    ...
    const fileName = `timetable_${selectedClass.nameEn.replace(/\s/g, '_')}.png`;
  ...
  const handleSendWhatsApp = async () => {
    ...
    if (blob) {
        ...
        if (copied) {
            ...
            const shared = await saveAndShareFile(blob, fileName, 'Timetable', 'Please share this with the ' + selectedClass.nameEn + ' class');
