...
const SettingsPanel: React.FC<{
    ...
    // Ultra Compact Vertical Layout Controls
        ...
        <div className="flex flex-col justify-between bg-white p-1 rounded border border-gray-200 shadow-sm h-full min-h-[34px]">
            ...
            <div className="flex items-center bg-gray-50 rounded border border-gray-200 p-0 overflow-hidden h-5 w-full">
                <button 
                    ...
                    className="w-4 flex items-center justify-center bg-white hover:bg-gray-100 text-gray-700 text-[9px] font-bold h-full border-r border-gray-200"
                >-</button>
                <input 
                    type="number" 
                    value={value} 
                    ...
                    className="flex-grow w-0 text-center text-[10px] font-bold text-gray-800 bg-transparent outline-none p-0 appearance-none m-0 leading-none"
                />
                <button 
                    onClick={() => handleValueChange(path, Math.min(max, parseFloat((value + step).toFixed(2))))}
                    className="w-4 flex items-center justify-center bg-white hover:bg-gray-100 text-gray-700 text-[9px] font-bold h-full border-l border-gray-200"
                >+</button>
            </div>
        </div>
    );
            ...
            <div className="flex flex-col justify-between bg-white p-1 rounded border border-gray-200 shadow-sm h-full min-h-[34px]">
                 ...
                 <div className="relative w-full">
                     <select 
                        ...
                        className={`w-full h-5 bg-gray-50 border border-gray-200 rounded text-[10px] font-medium px-0.5 py-0 focus:ring-1 focus:ring-teal-500 outline-none leading-none ${isFont ? 'cursor-pointer' : ''}`}
                        style={isFont ? { fontFamily: value } : {}}
                     >
                        {opts.map((o: any) => (
                            <option key={o.value} value={o.value} style={isFont ? { fontFamily: o.value, fontSize: '12px' } : {}}>
                                {o.label}
                            </option>
                        ))}
                     </select>
    ...
    const ColorControl = ({ label, path, value }: any) => (
        <div className="flex flex-col justify-between bg-white p-1 rounded border border-gray-200 shadow-sm h-full min-h-[34px]">
             <label className="text-[9px] font-extrabold text-gray-600 uppercase tracking-tight leading-none mb-0.5 break-words w-full truncate" title={label}>{label}</label>
             <div className="relative w-full h-5 rounded border border-gray-300 overflow-hidden">
    ...
    const ToggleControl = ({ label, path, value }: any) => (
        <div className="flex flex-col justify-between bg-white p-1 rounded border border-gray-200 shadow-sm h-full min-h-[34px]">
             <label htmlFor={path} className="text-[9px] font-extrabold text-gray-600 uppercase tracking-tight leading-none mb-0.5 break-words cursor-pointer w-full truncate" title={label}>{label}</label>
             <div className="relative w-full h-5 flex items-center justify-between">
                <div className="relative inline-block w-6 h-3.5 align-middle select-none transition duration-200 ease-in">
                    <input type="checkbox" name={path} id={path} checked={value} onChange={e => handleValueChange(path, e.target.checked)} className="toggle-checkbox absolute block w-3.5 h-3.5 rounded-full bg-white border border-gray-300 appearance-none cursor-pointer checked:right-0 right-2.5 checked:border-teal-500"/>
                    <label htmlFor={path} className={`toggle-label block overflow-hidden h-3.5 rounded-full cursor-pointer ${value ? 'bg-teal-500' : 'bg-gray-300'}`}></label>
                </div>
                <span className="text-[8px] font-bold text-gray-500">{value ? 'ON' : 'OFF'}</span>
            </div>
        </div>
    );
            ...
            <div className="flex border-b border-gray-300 bg-white overflow-x-auto no-scrollbar">
                {tabs.map(tab => (
                    <button 
                        ...
                        className={`flex-1 py-2 px-1 text-[10px] font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === tab.id ? 'text-teal-600 border-b-2 border-teal-600 bg-teal-50' : 'text-gray-500 hover:bg-gray-50'}`}
                    >
                        {tab.label}
                    </button>
                ))}
                ...
                <div className="grid grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-1.5">
                    ...
                    {activeTab === 'page' && (
                        <>
                            ...
                            <NumberControl label="Rows" path="rowsPerPage" value={options.rowsPerPage} min={5} max={100} />
                            <SelectControl label="Mode" path="colorMode" value={options.colorMode} options={[{value: 'color', label: 'Color'}, {value: 'bw', label: 'B&W'}]} />
                            
                            <div className={sectionHeaderClass}>Paper</div>
                            ...
                            <SelectControl label="Orient" path="page.orientation" value={options.page.orientation} options={[{value: 'portrait', label: 'Port'}, {value: 'landscape', label: 'Land'}]} />
                            <NumberControl label="Watermark" path="page.watermarkOpacity" value={options.page.watermarkOpacity} min={0} max={1} step={0.05} />

                            <div className={sectionHeaderClass}>Margins</div>
                            <NumberControl label="Top" path="page.margins.top" value={options.page.margins.top} min={0} max={50} />
                            <NumberControl label="Bottom" path="page.margins.bottom" value={options.page.margins.bottom} min={0} max={50} />
                            <NumberControl label="Left" path="page.margins.left" value={options.page.margins.left} min={0} max={50} />
                            <NumberControl label="Right" path="page.margins.right" value={options.page.margins.right} min={0} max={50} />
                        </>
                    )}

                    {activeTab === 'header' && (
                        <>
                            <div className={sectionHeaderClass}>Name</div>
                            <SelectControl label="Font" path="header.schoolName.fontFamily" value={options.header.schoolName.fontFamily} options={fontOptions} />
                            <NumberControl label="Size" path="header.schoolName.fontSize" value={options.header.schoolName.fontSize} min={10} max={60} />
                            <SelectControl label="Align" path="header.schoolName.align" value={options.header.schoolName.align} options={[{value: 'left', label: 'Left'}, {value: 'center', label: 'Center'}, {value: 'right', label: 'Right'}]} />
                            ...
                            <SelectControl label="Font" path="header.details.fontFamily" value={options.header.details.fontFamily} options={fontOptions} />
                            <NumberControl label="Size" path="header.details.fontSize" value={options.header.details.fontSize} min={8} max={24} />
                            <ColorControl label="Bg" path="header.bgColor" value={options.header.bgColor} />
                            
                            <div className={sectionHeaderClass}>Logo</div>
                            <ToggleControl label="Show" path="header.showLogo" value={options.header.showLogo} />
                            <NumberControl label="Size" path="header.logoSize" value={options.header.logoSize} min={20} max={200} />
                            <SelectControl label="Pos" path="header.logoPosition" value={options.header.logoPosition} options={[{value: 'left', label: 'Left'}, {value: 'center', label: 'Center'}, {value: 'right', label: 'Right'}]} />
                            
                            <div className={sectionHeaderClass}>Title</div>
                            <ToggleControl label="Show" path="header.showTitle" value={options.header.showTitle} />
                            <NumberControl label="Size" path="header.title.fontSize" value={options.header.title.fontSize} min={10} max={40} />
                            <ToggleControl label="Line" path="header.divider" value={options.header.divider} />
                        </>
                    )}
                    ...
                    {activeTab === 'table' && (
                        <>
                            ...
                            <SelectControl label="Font" path="table.fontFamily" value={options.table.fontFamily} options={fontOptions} />
                            <NumberControl label="Size" path="table.fontSize" value={options.table.fontSize} min={8} max={24} />
                            <NumberControl label="Pad" path="table.cellPadding" value={options.table.cellPadding} min={0} max={20} />
                            <ColorControl label="Border" path="table.borderColor" value={options.table.borderColor} />
                            
                            <div className={sectionHeaderClass}>Colors</div>
                            <ColorControl label="Head BG" path="table.headerBgColor" value={options.table.headerBgColor} />
                            <ColorControl label="Head Txt" path="table.headerColor" value={options.table.headerColor} />
                            <ColorControl label="Body BG" path="table.bodyBgColor" value={options.table.bodyBgColor} />
                            <ColorControl label="Body Txt" path="table.bodyColor" value={options.table.bodyColor || '#000000'} />
                            <ColorControl label="Stripe" path="table.altRowColor" value={options.table.altRowColor} />
                            
                            <div className={sectionHeaderClass}>Columns</div>
                            <NumberControl label="P.Width" path="table.periodColumnWidth" value={options.table.periodColumnWidth} min={20} max={100} />
                            <ColorControl label="P.BG" path="table.periodColumnBgColor" value={options.table.periodColumnBgColor} />
                        </>
                    )}

                    {activeTab === 'footer' && (
                        <>
                            <div className={sectionHeaderClass}>Setup</div>
                            <ToggleControl label="Show" path="footer.show" value={options.footer.show} />
                            <ToggleControl label="Page #" path="footer.includePageNumber" value={options.footer.includePageNumber} />
                            ...
                            <div className={sectionHeaderClass}>Style</div>
                            <SelectControl label="Font" path="footer.fontFamily" value={options.footer.fontFamily} options={fontOptions} />
                            <NumberControl label="Size" path="footer.fontSize" value={options.footer.fontSize} min={8} max={20} />
                            <ColorControl label="Color" path="footer.color" value={options.footer.color} />
                        </>
                    )}
                </div>
            </div>
...
const PrintPreview: React.FC<PrintPreviewProps> = ({ t, isOpen, onClose, title, generateHtml, onGenerateExcel, fileNameBase, children, designConfig, onSaveDesign }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (availableWidth < pageWidth) {
          fitZoom = Math.floor((availableWidth / pageWidth) * 100);
          fitZoom = Math.max(35, fitZoom); 
      }
  ...
  const saveCurrentPageEdits = () => {
      if (contentRef.current && pages[currentPage] !== undefined) {
  ...
  useEffect(() => {
    if (isOpen) {
      const content = generateHtml(lang, options);
      const generatedPages = Array.isArray(content) ? content : (content ? [content] : []);
      setPages(generatedPages);
      setCurrentPage(p => Math.min(p, Math.max(0, generatedPages.length - 1)));
    }
  }, [isOpen, lang, options, generateHtml]);
  ...
  const handleContentClick = (e: React.MouseEvent) => {
      ...
      if (target.closest('th')) {
          handleSort(table, colIndex, cell);
      }
  };
  ...
  const handleSort = (table: HTMLTableElement, colIndex: number, th: HTMLElement) => {
      ...
      if (hasRowSpans) {
          alert("Sorting is disabled for tables with merged cells (rowspans).");
          return;
      }
  ...
  const applyStyleToColumn = (styleProp: string, value: string) => {
      ...
      const applyRecursively = (el: HTMLElement) => {
          ...
          if (styleProp === 'color' || styleProp === 'fontWeight' || styleProp === 'fontStyle' || styleProp === 'textAlign') {
              ...
              children.forEach((child: any) => {
                  if (child.style) child.style[styleProp as any] = value;
              });
          }
      };

      if (activeCell) {
          applyRecursively(activeCell);
      }
      ...
      if(thead) {
          ...
          if(ths[activeColumn.colIndex]) {
              applyRecursively(ths[activeColumn.colIndex] as HTMLElement);
          }
      }

      const rows = table.querySelectorAll('tbody tr');
      rows.forEach(row => {
          const cell = row.cells[activeColumn.colIndex];
          if (cell && cell !== activeCell) {
              applyRecursively(cell);
          }
      });
  };
  ...
  const handlePrint = () => {
    ...
    setTimeout(() => {
        ...
        if (printWindow) {
          ...
          const grayscaleStyle = options.colorMode === 'bw' ? '<style>@media print { body { -webkit-print-color-adjust: exact; filter: grayscale(100%); } }</style>' : '';
          const printCss = '<style>@media print { body { margin: 0; padding: 0; } .print-container { page-break-after: always; } .print-container:last-child { page-break-after: auto; } }</style>';
          
          printWindow.document.write(grayscaleStyle + printCss + content);
              ...
              onclone: (clonedDoc: Document) => {
                  ...
                  style.innerHTML = `
                    * { 
                    ...
                    /* Important: Force layout and font for capture */
                    .print-container { 
                        font-family: '${options.table.fontFamily}', 'Jameel Noori Nastaleeq', 'Noto Nastaliq Urdu', serif !important; 
                    }
                    table, th, td, .font-urdu { 
  ...
  const handleZoom = (amount: number) => {
    setZoomLevel(prev => Math.max(10, Math.min(200, prev + amount)));
  };
                ...
                <div className="flex items-center justify-end gap-x-3 flex-wrap w-full sm:w-auto">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex bg-gray-800 rounded-lg p-0.5 border border-gray-700">
                            {(['en', 'ur', 'both'] as const).map((l) => (
                                <button
                                    key={l}
                                    onClick={() => setLang(l)}
                                    className={`px-2 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all leading-none ${
                                        lang === l 
                                        ? 'bg-teal-600 text-white shadow-sm' 
                                        : 'text-gray-400 hover:text-gray-200 hover:bg-gray-700'
                                    }`}
                                >
                                    {l === 'en' ? 'ENG' : l === 'ur' ? 'اردو' : 'ALL'}
                                </button>
                            ))}
                        </div>
                        ...
                        <button 
                            ...
                            className={`p-2 rounded-lg transition-all border ${isSettingsOpen ? 'bg-teal-600 border-teal-500 text-white shadow-inner' : 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-700 hover:text-white'}`}
                        >
                            <SettingsIcon />
                        </button>
                    </div>
                    ...
                    {/* Exit */}
                    <button onClick={onClose} title={t.close} className="p-2 rounded-lg bg-red-600/10 hover:bg-red-600 text-red-400 hover:text-white border border-red-900/50 hover:border-red-500 transition-all">
                        <CloseIcon />
                    </button>
                </div>
                
                {/* Mobile Extra Children */}
