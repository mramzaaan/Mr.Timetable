...
const SchoolInfoModal: React.FC<SchoolInfoModalProps> = ({ t, isOpen, onClose, schoolConfig, onUpdateSchoolConfig }) => {
  ...
  const inputStyleClasses = "mt-1 block w-full px-4 py-3 bg-[var(--bg-tertiary)]  rounded-[2rem]  text-[var(--text-primary)] placeholder-[var(--text-placeholder)] focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm transition-all";
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[120] p-4 animate-fade-in" onClick={onClose}>
        <div className="bg-white/60 dark:bg-black/20 backdrop-blur-[30px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/50 dark:border-white/10 rounded-[2.5rem]  w-full max-w-2xl transform transition-all  overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-8 space-y-8">
                {feedback.message && (
                    <div className={`p-4 rounded-[2rem] text-sm font-bold animate-scale-in ${feedback.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`} role="alert">
                        {feedback.message}
                    </div>
                )}
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-6 p-4 bg-[var(--bg-tertiary)]/50 rounded-[2rem] ">
                            {localSchoolLogo ? (
                                <div className="relative group flex-shrink-0">
                                    <img src={localSchoolLogo} alt="School Logo" className="h-24 w-24 object-contain rounded-[2rem] border-2 border-white  bg-white" />
                                    <button onClick={handleRemoveLogo} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1.5  hover:bg-red-600 transition-all transform hover:scale-110">
                                ...
                                <div className="h-24 w-24 flex flex-col items-center justify-center rounded-[2rem] border-2 border-dashed border-[var(--border-secondary)] bg-[var(--bg-tertiary)] text-[var(--text-placeholder)] flex-shrink-0 gap-1">
                            ...
                            <div className="flex-1 w-full">
                                <label htmlFor="logo-upload-modal" className="flex flex-col items-center justify-center w-full px-4 py-3 bg-white/60 dark:bg-black/20 backdrop-blur-[30px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/50 dark:border-white/10  rounded-[2rem] cursor-pointer hover:border-indigo-500 transition-colors group">
            ...
            <div className="p-8 pt-0 flex justify-end gap-4">
                <button onClick={onClose} className="px-8 py-3 text-sm font-black uppercase tracking-widest text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)] rounded-[2rem] transition-all">{t.cancel}</button>
                <button 
                    onClick={handleSettingsSave} 
                    className="px-10 py-3 bg-indigo-600 text-white font-black uppercase tracking-[0.2em] rounded-[2rem]  hover:shadow-indigo-500/40 transition-all transform hover:-translate-y-1 active:scale-[0.98]"
                >
                    {t.saveSettings}
                </button>
            </div>
